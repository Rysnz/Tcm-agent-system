...
### 14.1 当前支持的 Embedding 模型

系统已内置对以下主流中文 Embedding 模型的支持（通过 `apps/knowledge/` 管理）：

| 模型名称 | 维度 | 说明 |
|---|---|---|
| `shibing624/text2vec-base-chinese` | 768 | 推荐，中文语义相似度优化 |
| `BAAI/bge-large-zh-v1.5` | 1024 | 高精度，显存需求较大 |
| `BAAI/bge-small-zh-v1.5` | 384 | 轻量版，适合资源受限环境 |
| `m3e-base` | 768 | 多语言嵌入，较好的跨语种效果 |
| `m3e-small` | 384 | m3e 轻量版 |
...
2. **在知识库设置界面选择 Embedding 模型**
   - 进入后台管理 → 知识库 → 点击某知识库 → 设置
   - 在"Embedding 模型"下拉框中选择合适的模型
   - 系统会根据选择自动设置向量维度

3. **确保 pgvector 扩展已安装**（PostgreSQL 环境）
   ```sql
   CREATE EXTENSION IF NOT EXISTS vector;
   ```

4. **重新处理文档以生成向量**
   - 在知识库文档列表中选择文档并点击"重新处理"
   - 系统会使用新的 Embedding 模型重新生成向量并写入数据库

### 14.3 首次使用知识库

首次使用时，Embedding 模型会自动从 HuggingFace Hub 下载（约 400MB–1.5GB）。
如在离线环境中部署，可预先下载模型并配置本地路径：

```python
# apps/knowledge/services/embedding.py
EMBEDDING_MODEL_PATH = "/data/models/text2vec-base-chinese"
```

### 14.4 验证 Embedding 是否正常工作
...
>>> v = m.encode("感冒发烧")
>>> print(v.shape)  # 应输出 (768,)
```

如输出正确维度，则 Embedding 环境配置完毕。

---

*文档版本：v2.1 | 生成时间：2026-03-20 | 项目：Tcm-agent-system*
