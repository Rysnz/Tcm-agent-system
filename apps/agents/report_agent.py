...
class ReportAgent(BaseAgent):
    ...
    def _execute(self, state: SessionState, **kwargs) -> SessionState:
        ...
        if not data:
            raise ValueError("ReportAgent 未获取到可解析的结构化输出")
    ...
    def _format_readable_report(
        ...
        # 患者信息 - 使用 state 中的实际信息，而不是 LLM 生成的
        # 即使信息不完整，也使用实际信息，避免 LLM 幻觉
        patient_lines = []
        if state.chief_complaint:
            patient_lines.append(f"主诉：{state.chief_complaint}")
        if state.patient_profile.gender:
            patient_lines.append(f"性别：{state.patient_profile.gender}")
        if state.patient_profile.age_group:
            patient_lines.append(f"年龄段：{state.patient_profile.age_group}")
        if state.symptoms:
            patient_lines.append(f"症状：{', '.join(s.name for s in state.symptoms[:5])}")
        if patient_lines:
            parts.append(f"👤 患者信息\n{chr(10).join(patient_lines)}\n")
        else:
            # 如果 state 中没有信息，显示"未提供"，而不是使用 LLM 幻觉的信息
            parts.append(f"👤 患者信息\n未提供详细信息\n")
        ...
        # 辨证结论 - 优先从 state.syndrome_candidates 中获取
        parts.append("🎯 辨证结论")
        
        # 方法1: 从 state.syndrome_candidates 中获取
        if state.syndrome_candidates:
            primary = state.syndrome_candidates[0]
            parts.append(f"主证型：{primary.name}")
            if primary.confidence:
                parts.append(f"置信度：{primary.confidence:.0%}")
            if primary.supporting_symptoms:
                parts.append("支持依据：")
                for ev in primary.supporting_symptoms[:5]:
                    parts.append(f"  • {ev}")
            if primary.reasoning:
                parts.append(f"辨证分析：{primary.reasoning}")
            parts.append("")
        # 方法2: 从 LLM 返回的 JSON 中获取
        elif data.get("syndrome_conclusion"):
            syndrome = data["syndrome_conclusion"]
            ...
            if evidence:
                ...
                for ev in evidence[:4]:
                    parts.append(f"  • {ev}")
            parts.append("")
        # 方法3: 从 state.primary_syndrome 中获取
        elif state.primary_syndrome:
            parts.append(f"主证型：{state.primary_syndrome}")
            parts.append("")
