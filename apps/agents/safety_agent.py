...
class SafetyGuardAgent(BaseAgent):
    ...
    def __init__(self, llm_caller=None, quick_check: bool = False, stage: ConsultStage = None):
        super().__init__(llm_caller)
        self.quick_check = quick_check
        # 如果指定了 stage，则覆盖默认的 stage
    ...
    def _execute(self, state: SessionState, **kwargs) -> SessionState:
        ...
        # 6. 完整模式：默认不做LLM语义审查（本地模型耗时高）
        enable_llm_review = bool(kwargs.get("enable_llm_review", False))
        if enable_llm_review and (not self.quick_check) and new_state.recommendations:
