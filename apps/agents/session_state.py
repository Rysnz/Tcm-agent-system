...
class TongueDiagnosis(BaseModel):
    """舌象诊断结论"""
    summary: str = Field(default="", description="舌象诊断总结（通俗易懂）")
    indications: List[str] = Field(default_factory=list, description="可能提示的身体状况")
    suggestions: List[str] = Field(default_factory=list, description="针对舌象的调理建议")
...
class ObservationData(BaseModel):
    ...
    diagnosis: TongueDiagnosis = Field(default_factory=TongueDiagnosis, description="舌象诊断结论")
...
class SyndromeCandidate(BaseModel):
    ...
    reasoning: str = Field(default="", description="辨证推理过程")
    classical_basis: Optional[str] = Field(default=None, description="经典依据")
