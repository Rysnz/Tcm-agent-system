...
class SessionStateTest(TestCase):
    ...
    def test_mark_high_risk(self):
        ...
        self.assertIn("胸痛", state.safety_result.safety_message or "")
...
class OrchestratorTest(TestCase):
    ...
    def test_inquiry_should_not_jump_to_syndrome_on_first_real_reply(self):
        """首轮有效回答后不应直接推进到辨证阶段"""
        from apps.agents.inquiry_agent import InquiryAgent

        mock_response = '''
        {
          "questions": ["请问您平时食欲怎么样？"],
          "is_sufficient": true,
          "new_symptoms": [],
          "answered_dimensions": ["寒热", "头身"],
          "inquiry_answers": {
            "寒热": "手脚冰冷",
            "头身": "偶尔头疼"
          }
        ...
        agent = InquiryAgent(llm_caller=MagicMock(return_value=mock_response))
        state = SessionState()
        state.current_stage = ConsultStage.INQUIRY
        state.chief_complaint = "手脚冰冷，偶尔头疼"
        state.pending_questions = [
            "请问您有没有感觉怕冷或发热？体温多少？",
            "请问您有没有头痛、头晕或身体某部位疼痛？",
        ]
        state.add_message("assistant", "1. 请问您有没有感觉怕冷或发热？体温多少？\n2. 请问您有没有头痛、头晕或身体某部位疼痛？")
        state.add_message("user", "我手脚冰冷，偶尔会头疼，持续好几年了")

        result = agent.run(state)

        self.assertEqual(result.current_stage, ConsultStage.INQUIRY)
        self.assertTrue(len(result.pending_questions) > 0)
        self.assertIn("寒热", result.inquiry_answers)
        self.assertIn("头身", result.inquiry_answers)

    def test_synthesized_pending_only_in_inquiry_stage(self):
        """非INQUIRY阶段不应回放pending_questions为助手回复"""
        orch = TCMOrchestrator(llm_caller=MagicMock(return_value="{}"))
        state = orch.create_session()
        state.current_stage = ConsultStage.SAFETY_CHECK
        state.pending_questions = ["您是否有反酸、烧心或胃部灼热感？"]
        state.add_message("assistant", "1. 您是否有反酸、烧心或胃部灼热感？")
        before_count = len([m for m in state.messages if m.get("role") == "assistant"])

        result = orch.process_message(state, "没有")
        after_msgs = [m for m in result.messages if m.get("role") == "assistant"]

        self.assertGreaterEqual(len(after_msgs), before_count)
        self.assertNotEqual(
            after_msgs[-1].get("content", "").strip(),
            "1. 您是否有反酸、烧心或胃部灼热感？",
        )
...
class SessionPersistenceTest(TestCase):
    ...
    def test_save_and_retrieve_session(self):
        ...
        # 检索并验证
        retrieved = ConsultationSession.objects.get(session_id=state.session_id)
        self.assertEqual(retrieved.chief_complaint, "头痛")
        self.assertEqual(retrieved.current_stage, "inquiry")
        
        # 反序列化状态
        restored_state = TCMOrchestrator.restore_state(retrieved.state_data)
        self.assertEqual(restored_state.chief_complaint, "头痛")
        self.assertEqual(restored_state.current_stage, ConsultStage.INQUIRY)
...
class AgentFallbackTest(TestCase):
    """Agent降级策略测试"""
    
    def test_intake_agent_fallback(self):
        """测试IntakeAgent降级策略"""
        from apps.agents.intake_agent import IntakeAgent
        ...
        agent = IntakeAgent(llm_caller=failing_llm)
        agent.max_retries = 0  # 不重试
        
        state = SessionState()
        state.add_message("user", "我头疼很厉害")
        ...
        # 验证没有强制建议就医（技术错误不应强制就医）
        last_msg = assistant_msgs[-1]["content"]
        self.assertNotIn("请立即就医", last_msg)
...
class InputValidationTest(TestCase):
    ...
    def test_message_length_limit(self):
        ...
        from apps.agents.views import send_message
        
        # 创建超长消息
        long_message = "测试" * 2000  # 4000字符
        
        factory = RequestFactory()
        request = factory.post(
            '/api/v2/consult/message/',
            {'message': long_message, 'session_id': 'test'},
            content_type='application/json'
        )
        
        # 注意：这个测试需要完整的Django REST framework环境
        # 这里只是展示测试思路
...
class SafetyEnhancedTest(TestCase):
    """增强安全审查测试"""
    
    def test_mixed_critical_keywords(self):
        """测试混合高危关键词检测"""
        state = self._make_state_with_complaint("我突然感觉胸痛，而且呼吸困难")
        agent = SafetyGuardAgent(quick_check=True)
        result = agent.run(state)
        
        self.assertTrue(result.is_high_risk)
        self.assertIn("胸痛", result.safety_result.triggered_keywords)
        self.assertIn("呼吸困难", result.safety_result.triggered_keywords)
