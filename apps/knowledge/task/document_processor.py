...
class DocumentProcessor:
    ...
    def _extract_word(self, file_path: str) -> list:
        ...
        if file_ext == '.docx':
            ...
            # 清理中文字符间的空格
            full_text = self._clean_chinese_text(full_text)
            if full_text.strip():
    ...
    def _extract_text_file(self, file_path: str) -> list:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                content = file.read()
                # 清理中文字符间的空格
                content = self._clean_chinese_text(content)
                if content.strip():
    ...
    def _smart_split(self, text: str, max_length: int = 1000, title: str = '') -> list:
        """
        智能分段算法（使用LangChain CharacterTextSplitter）
        :param text: 待分段的文本
        ...
        try:
            # 使用LangChain的CharacterTextSplitter进行分段
            splitter = CharacterTextSplitter(
                separator="\n",  # 按换行符分隔
                chunk_size=max_length,  # 最大段落长度
            ...
            logger.info(f"LangChain分段完成: {len(paragraphs)} 个段落")
            
        except Exception as e:
            logger.error(f"LangChain分段失败，使用备用方案: {e}")
            # 备用方案：按最大长度分段
