...
class KnowledgeBaseViewSet(viewsets.ModelViewSet):
    ...
    def stats(self, request, pk=None):
        ...
        vector_count = Embedding.objects.filter(knowledge=knowledge_base).count()
...
class DocumentViewSet(viewsets.ModelViewSet):
    ...
    def paragraphs(self, request, pk=None):
        ...
        logger.info(f"Getting paragraphs for document: {document.id} - {document.name}")
        
        # 不过滤is_delete，显示所有段落
        paragraphs = Paragraph.objects.filter(document=document).order_by('page_number', 'id')
        
        paragraph_count = paragraphs.count()
        logger.info(f"Found {paragraph_count} paragraphs")
        
        paragraphs_list = []
        for p in paragraphs[:100]:  # 限制最多返回100个段落
    ...
    def post(self, request):
        ...
        if not results:
            return Response({
                ...
                'results_count': 0,
                'results': [],
                'message': 'No results found. Try a different query or search type.'
            })
