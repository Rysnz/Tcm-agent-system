...
export const knowledgeApi = {
  ...
  searchKnowledge: (data: { knowledge_base_id: string; query: string; search_type?: string; top_k?: number }) => 
    request.post('/knowledge/search/', data),
  
  // 获取知识库统计信息
  getKnowledgeBaseStats: (knowledgeBaseId: string) => 
    request.get(`/knowledge/knowledge_base/${knowledgeBaseId}/stats/`),
  
  // 重新处理文档
  reprocessDocument: (documentId: string) => 
    request.post(`/knowledge/document/${documentId}/reprocess/`, {}),
  
  // 获取文档段落
  getDocumentParagraphs: (documentId: string) => 
    request.get(`/knowledge/document/${documentId}/paragraphs/`),
}
...
export interface ConsultMessageResponse {
  ...
  agent_steps: Array<{ agent: string; stage: string; success: boolean; retry_count: number; step_note?: string }>
  report?: ConsultReport | null
  model_error?: boolean
}

export interface StreamEvent {
  type: 'stage' | 'token' | 'done' | 'error' | 'agent_step'
  stage?: string
  message?: string
  content?: string
  is_high_risk?: boolean
...
export const consultApi = {
  ...
  sendMessageStream: async (
    ...
    const response = await fetch('/api/v2/consult/message/stream/', {
      method: 'POST',
      headers: {
    ...
    if (!response.ok || !response.body) {
      // 如果是401错误，检查是否真的需要重新登录
      if (response.status === 401) {
        // 只有在token确实过期时才清除
        // 先不立即清除token，让系统自然处理
        throw new Error('认证失败，请检查登录状态')
      }
      throw new Error(`Stream请求失败: ${response.status}`)
    }
    ...
    if (lastChunk.startsWith('data:')) {
      const payload = lastChunk.slice(5).trim()
      try {
        const evt = JSON.parse(payload)
        onEvent(evt)
      } catch {
...
export const authApi = {
  ...
  getArchives: (params?: { q?: string; start?: string; end?: string }) => request.get('/auth/archives/', { params: params || {} }),
  getConsultTimeline: (sessionId: string) => request.get(`/auth/archives/consult/${sessionId}/timeline/`),
  getAdminStats: () => request.get<{ registeredUserCount: number }>('/auth/admin/stats/'),
  saveWellnessArchive: (data: any) => request.post('/auth/archives/wellness/', data),
  saveTongueArchive: (data: any) => request.post('/auth/archives/tongue/', data),
  saveConsultArchive: (data: any) => request.post('/auth/archives/consult/', data),
  saveWellnessCheckin: (data: any) => request.post('/auth/archives/wellness/checkin/', data),
  // 获取舌象档案列表
  getTongueArchives: (): Promise<{ results: any[] }> => request.get('/auth/archives/tongue/'),
  // 获取养生计划档案列表
  getWellnessArchives: (): Promise<{ results: any[] }> => request.get('/auth/archives/wellness/'),
}
...
export const modelApi = {
  ...
  // 获取模型列表
  getModelList: async (provider: string, modelType: string, baseUrl?: string) => {
    try {
      ...
      return await request.get<ModelListItem[]>(`/model/model-list/?${query.toString()}`)
    } catch (error) {
      console.error('获取模型列表失败:', error)
      return []
    }
  ...
  validateCredential: async (
    ...
    try {
      ...
      return { is_valid: false, error: responseData.error || responseData.detail || '连接校验失败' }
    }
  },
  ...
  getAgentConfig: async () => {
    try {
      return await request.get<AgentModelConfig[]>('/model/agent-config/')
    } catch (error) {
  ...
  saveAgentConfig: async (data: Array<{ agent_name: string; model: string | null }>) => {
    try {
      return await request.put<AgentModelConfig[]>('/model/agent-config/', data)
    } catch (error) {
      console.error('保存Agent模型配置失败:', error)
      throw error
    }
  }
}
